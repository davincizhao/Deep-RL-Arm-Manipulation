...
#define REPLAY_MEMORY 2000//10000
#define BATCH_SIZE 4  //8
#define USE_LSTM true
#define LSTM_SIZE 32 //32
#define NUM_ACTION 6 //3 joint ,every joint have two choises.
...
void ArmPlugin::Load(physics::ModelPtr _parent, sdf::ElementPtr /*_sdf*/) 
{
	...
	*/
	...
	//gazebo::transport::SubscriberPtr sub = cameraNode->Subscribe("cameraNode", this->onCameraMsg(), class_instance);
...
bool ArmPlugin::createAgent()
{
	...
	*/
	...
	agent = dqnAgent::Create(INPUT_WIDTH, INPUT_HEIGHT, INPUT_CHANNELS, NUM_ACTION, 
...
bool ArmPlugin::updateAgent()
{
	...
	*/
	...
	/
	*/
	float joint = 1.0; // TODO - Set joint position based on whether action is even or odd.
	if( action %2 == 0)
	{
		...
		*/ 
		...
		if(!checkGroundContact)
		{
			...
			if( episodeFrames > 1 )
			{
				...
				rewardHistory = -abs(gripBBox.min.z - propBBox.max.z); // robot arm touching the object,because only joint 1 and joint 2 control,so just calculate Z axis distance.
				//rewardHistory = 10*avgGoalDelta - distGoal; //arm's gripper base touching the object
				newReward     = true;	
			}
